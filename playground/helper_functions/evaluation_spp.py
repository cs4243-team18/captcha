...
def evaluate_character_performance(
    ...
    with torch.no_grad():
        predictions = []
        for img in X_test:
            output = trained_model(img.unsqueeze(0))
            predictions.append(output)
        predictions = torch.cat(predictions, dim=0)

    y_pred = torch.argmax(predictions, axis=1).cpu().numpy()
    y_test = torch.tensor([label.item() for label in y_test]).cpu().numpy()
...
def evaluate_captcha_performance(
    ...
    for captcha_x, captcha_y in zip(X_test_captcha, y_test_captcha):
        with torch.no_grad():
            preds = []
            for char_img in captcha_x:
                output = trained_model(char_img.unsqueeze(0))  # add batch dimension: [1, C, H, W]
