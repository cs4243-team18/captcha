...
from .segmentation import segment_by_projection_v1, segment_by_projection_v1_proportionate, segment_by_projection_v1_with_padding, segment_by_projection_v2, segment_by_projection_v3
from .segmentation import IMG_HEIGHT, IMG_WIDTH
"""
...
def _visualize_resized_imgs(char_imgs: list, correct_characters: str):
    ...
    for i, char in enumerate(correct_characters):
        if char in ['o', '0', '1', 'l', 'i']:
            ...
            a[0].set_title(
                f"original {char} from {''.join(correct_characters)}")
            a[0].axis('off')
...
def _get_transformed_data_helper(folder_path, is_train, segmentation_function):
    ...
    for filename in tqdm(filenames, desc=desc):
        ...
        y.extend([CHARACTERS.index(char_label)
                 for char_label in correct_characters])
...
def _get_transformed_data_helper_without_resize(folder_path, is_train, segmentation_function):
    """
    Segments input X based on segmentation_function and returns (X, y) as lists of individual tensors.
    """
    ...
    for filename in tqdm(filenames, desc=desc):
        ...
        for img, char_label in zip(char_imgs, correct_characters):
            img_tensor = torch.tensor(img, dtype=torch.float32).unsqueeze(0) / 255.0  # (1, H, W)
            X.append(img_tensor)
            y.append(torch.tensor(CHARACTERS.index(char_label), dtype=torch.long))
...
def get_transformed_data_for_captcha_evaluation_helper_without_resize(folder_path, segmentation_function) -> tuple[list[torch.Tensor], list[torch.Tensor], tuple[int, int]]:
    """
    Rationale is to allow for evaluation of CAPTCHA performance now instead of just individual characters, which includes 
    number of failed segmentations.
    Now returns (X,y) with an additional layer of nesting for grouping character images and their labels for each CAPTCHA.
    ...
    for filename in tqdm(filenames, desc=desc):
        ...
        # Treat each CAPTCHA as a (X,y) dataset of character images and labels
        X = [img for img in char_imgs]
        y = [CHARACTERS.index(char_label) for char_label in correct_characters]

        # Transform (X,y) for each CAPTCHA to tensors for PyTorch, and add them to X_test_captcha, y_test_captcha
        X = [torch.tensor(img, dtype=torch.float32).unsqueeze(0) / 255.0 for img in X]
        y = torch.tensor(y, dtype=torch.long)
        X_test_captcha.append(X)
        y_test_captcha.append(y)
...
def get_transformed_data(folder_path, is_train) -> tuple[np.ndarray, np.ndarray, list[str], np.ndarray, StandardScaler]:
    ...
    for filename in tqdm(all_images, desc="Preparing Data"):
        ...
        image = cv2.imread(img_path)
        preprocessed_image = preprocess_image_v1(image)
        character_boundaries = segment_by_projection_v2(
            preprocessed_image, PROJECTION_THRESHOLD)
...
def get_transformed_data_v1_proportionate(folder_path, is_train) -> tuple[torch.Tensor, torch.Tensor]:
    return _get_transformed_data_helper(folder_path, is_train, segment_by_projection_v1_proportionate)
...
def get_transformed_data_v2_without_resize(folder_path, is_train) -> tuple[torch.Tensor, torch.Tensor]:
    return _get_transformed_data_helper_without_resize(folder_path, is_train, segment_by_projection_v1)
...
def get_transformed_data_for_captcha_evaluation_v1_proportionate(folder_path) -> tuple[list[torch.Tensor], list[torch.Tensor], tuple[int, int]]:
    return get_transformed_data_for_captcha_evaluation_helper(folder_path, segment_by_projection_v1_proportionate)
...
def get_transformed_data_for_captcha_evaluation_without_resize(folder_path) -> tuple[list[torch.Tensor], list[torch.Tensor], tuple[int, int]]:
    return get_transformed_data_for_captcha_evaluation_helper_without_resize(folder_path, segment_by_projection_v1)
